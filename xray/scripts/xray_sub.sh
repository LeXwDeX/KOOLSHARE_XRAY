...
get_oneline_rule_now(){
	...
	if [ "$v2ray_basic_suburl_socks" == "1" ];then
		...
		if [ -n "$socksopen" ];then
			...
			curl --connect-timeout 8 -s -L --socks5-hostname 127.0.0.1:1280 $v2ray_subscribe_link > /tmp/xray_subscribe_file.txt
		else
			echo_date "没有可用的socks5代理端口，改用常规网络下载..."
			curl --connect-timeout 8 -s -L $v2ray_subscribe_link > /tmp/xray_subscribe_file.txt
		fi
	else
		echo_date "使用常规网络下载..."
		curl --connect-timeout 8 -s -L $v2ray_subscribe_link > /tmp/xray_subscribe_file.txt
	...
	if [ "$?" == "0" ];then
		...
		if [ -n "$blank" ];then
			...
			if [ "`echo $v2ray_subscribe_link|grep ^https`" ];then
				wget --no-check-certificate -qO /tmp/xray_subscribe_file.txt $v2ray_subscribe_link
			else
				wget -qO /tmp/xray_subscribe_file.txt $v2ray_subscribe_link
			fi
		fi
		#下载为空...
...
set_cru(){
	if [ "$v2ray_basic_node_update" = "1" ];then
		...
		if [ "$v2ray_basic_node_update_day" = "7" ];then
			echo "0 $v2ray_basic_node_update_hr * * * /koolshare/scripts/xray_sub.sh 3 3 #v2raynodeupdate#" >> /etc/crontabs/root
			echo_date "设置自动更新订阅服务在每天 $v2ray_basic_node_update_hr 点。" >> $LOG_FILE
		else
			echo "0 $v2ray_basic_node_update_hr * * v2ray_basic_node_update_day /koolshare/scripts/xray_sub.sh 3 3 #v2raynodeupdate#" >> /etc/crontabs/root
