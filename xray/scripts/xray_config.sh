...
V2RAY_CONFIG_FILE="/koolshare/xray/xray.json"
set_lock(){
	exec 1000>"$LOCK_FILE"
	flock -x 1000
}
...
create_dnsmasq_conf(){
	...
	# append china site
	[ ! -f /tmp/dnsmasq.d/xraycdn.conf -a "$v2ray_dns_plan" == "2" ] && {
	...
	[ ! -f /tmp/dnsmasq.d/xraygfw.conf -a "$v2ray_dns_plan" == "1" -o "$v2ray_acl_default_mode" == "1" ] && {
	...
	if [ -n "$v2ray_dnsmasq" ];then
		...
		echo "$v2ray_dnsmasq" | base64_decode | sort -u >> /tmp/dnsmasq.d/xraycustom.conf
	fi
...
auto_start(){
	...
	uci -q batch <<-EOT
		...
		set firewall.ks_v2ray.path=/koolshare/scripts/xray_nat.sh
	...
	*)
	...
	if [ -n "$v2ray_server" -a "$v2ray_server" != "null" ];then
		...
		if [ -n "$IFIP_VS" ];then
			...
			echo "server=/$v2ray_server/114.114.114.114" > /tmp/dnsmasq.d/xray_server.conf
			...
			if [ -n "$v2ray_server_ip" ];then
				...
				#echo "address=/$v2ray_server/$v2ray_server_ip" > /tmp/dnsmasq.d/xray_host.conf
...
start_v2ray(){
	...
	cd /koolshare/bin
	v2ray --config=/koolshare/xray/xray.json >/dev/null 2>&1 &
...
check_update_v2ray(){
	...
	if [ "$v2ray_basic_check_releases" == "0" ]; then
		...
		lastver=$(get_latest_release "v2ray/xray-core")
	fi
	...
	if [ -n "$lastver" ]; then 
		...
		if [ "$lastver" == "$oldver" ]; then
			...
			if [ "$?" -eq 0 ] ; then
				...
				cp -rf /tmp/xray_update/v2ctl $KSROOT/bin/v2ctl
...
set_v2ray_watchdog(){
	if [ "$v2ray_basic_watchdog" == "1" ]; then
		...
		echo "*/$v2ray_basic_watchdog_time * * * * /koolshare/scripts/xray_config.sh watchdog #v2raywatchdog#" >> /etc/crontabs/root
...
set_v2ray_cron(){
	if [ "$v2ray_basic_cron" == "1" -a "$v2ray_basic_enable" == "1" ]; then
		...
		echo "$v2ray_basic_cron_enableminute $v2ray_basic_cron_enablehour * * * /koolshare/scripts/xray_config.sh #v2raytimeswitch#" >> /etc/crontabs/root
		echo "$v2ray_basic_cron_disableminute $v2ray_basic_cron_disablehour * * * /koolshare/scripts/xray_config.sh stop #v2raytimeswitch#" >> /etc/crontabs/root
...
7)
	...
	if [ "$v2ray_basic_rule_update" = "1" ];then
		...
		if [ "$v2ray_basic_rule_update_day" = "7" ];then
			echo "0 $v2ray_basic_rule_update_hr * * * /koolshare/scripts/xray_config.sh cron 6 #v2raynodeupdate#" >> /etc/crontabs/root
