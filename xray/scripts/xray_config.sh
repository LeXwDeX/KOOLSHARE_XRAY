...
V2RAY_CONFIG_FILE="/koolshare/xray/v2ray.json"
set_lock(){
	exec 1000>"$LOCK_FILE"
	flock -x 1000
}
...
create_dnsmasq_conf(){
	...
	[ ! -f /tmp/dnsmasq.d/v2raycdn.conf -a "$v2ray_dns_plan" == "2" ] && {
		...
		cat $KSROOT/xray/cdn.txt | sed "s/^/server=&\/./g" | sed "s/$/\/&$CDN/g" | sort | awk '{if ($0!=line) print;line=$0}' > /tmp/dnsmasq.d/v2raycdn.conf
	}
	[ ! -f /tmp/dnsmasq.d/v2raygfw.conf -a "$v2ray_dns_plan" == "1" -o "$v2ray_acl_default_mode" == "1" ] && {
		...
		ln -sf $KSROOT/xray/gfwlist.conf /tmp/dnsmasq.d/v2raygfw.conf
	}
...
gen_v2ray_config(){
	...
	if [ "$xray_basic_type" == "1" ]; then
		server_tag=$(dbus get "v2ray_server_tag_$xray_basic_server")
		server_config=$(dbus get "v2ray_server_config_$xray_basic_server")
	else
		server_tag=$(dbus get "v2ray_sub_tag_$xray_basic_server")
		server_config=$(dbus get "v2ray_sub_config_$xray_basic_server")
	fi
	...
	*)
	...
	if [ -n "$v2ray_server" -a "$v2ray_server" != "null" ];then
		...
		if [ -n "$IFIP_VS" ];then
			xray_basic_server_ip="$v2ray_server"
			...
			if [ -n "$v2ray_server_ip" ];then
				...
				xray_basic_server_ip="$v2ray_server_ip"
			else
...
start_v2ray(){
	...
	cd /koolshare/bin
	v2ray --config=/koolshare/xray/v2ray.json >/dev/null 2>&1 &
