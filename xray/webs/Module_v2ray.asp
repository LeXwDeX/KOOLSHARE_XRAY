...
.box, #v2ray_tabs {
	...
	<script type="text/javascript">
		...
		function auto_node_set(){
			...
			if (n1){
				...
				E("_xray_basic_config").value = Base64.decode(dbus["v2ray_server_config_" + nid ])
				if (!n2){
				...
				E("_xray_basic_config").value = Base64.decode(dbus["v2ray_sub_config_" + nid ])			
			}
			//console.log(n3);
		}
		...
		function join_node(){
			...
			if (server_type){ 
				if (typeof(dbus["xray_server_node_max"]) == "undefined"){
					...
					node_v2ray = parseInt(dbus["xray_server_node_max"]);
				}
				...
				E("_xray_basic_server").options[node_v2ray] = new Option("新增配置", node_v2ray + 1);
			}else{
				if (typeof(dbus["xray_sub_node_max"]) == "undefined"){
					node_v2ray = 0;
				}else{
					node_v2ray = parseInt(dbus["xray_sub_node_max"]);
				}
				for ( var i = 0; i < node_v2ray; i++){
		...
		function save(){
			...
			if (node_type){			
				dbus["v2ray_server_config_" + server_id] = Base64.encode(E('_xray_basic_config').value);
				...
				dbus["v2ray_sub_config_" + server_id] = Base64.encode(E('_xray_basic_config').value);
			}
		...
		function version_show() {
			$('#_v2ray_version').html( '<font color="#1bbf35">v2ray for openwrt - ' + (dbus["v2ray_version"]  || "") + '</font>' );
			$.ajax({
				url: 'https://raw.githubusercontent.com/HEXtoDEC/LEDE_V2Ray/master/config.json.js',
				type: 'GET',
				dataType: 'json',
	...
	<div class="box">
		<div class="heading">
			...
			<a href="https://raw.githubusercontent.com/HEXtoDEC/LEDE_V2Ray/master/Changelog.txt" target="_blank" class="btn btn-primary" style="float:right;border-radius:3px;margin-right:5px;margin-top:0px;">更新日志</a>
		...
		<div class="content" style="margin-top: -20px;">
			...
			<script type="text/javascript">
				$('#v2ray_basic_pannel').forms([
					...
					{ title: 'V2ray服务器类型', name:'xray_basic_type',type:'select',options:[['1', '自建'], ['2', '订阅']], value: dbus.xray_basic_type || '1'},
					{ title: 'V2ray服务器选择', name:'xray_basic_server',type:'select',options:option_server_list},
					...
					//{ title: '<b>v2ray配置文件</b></br></br><font color="#B2B2B2"># 此处填入v2ray json<br /># 请保证json内outbound的配置正确！</font>', name:'xray_basic_config',type:'textarea', value: do_js_beautify(Base64.decode(dbus.xray_basic_config))||"", style: 'width: 100%; height:450px;' },
					{ title: '<b>v2ray配置文件</b></br></br><font color="#B2B2B2"># 此处填入v2ray json<br /># 请保证json内outbound的配置正确！</font>', name:'xray_basic_config',type:'textarea', style: 'width: 100%; height:450px;' },
				]);
				join_node();
			</script>
		</div>
		...
		<div class="content" style="margin-top: -20px;">
			...
			<script type="text/javascript">
				$('#ss_rule_pannel').forms([
					{ title: 'gfwlist域名数量', rid:'gfw_number_1', text:'<a id="gfw_number" href="https://raw.githubusercontent.com/HEXtoDEC/LEDE_V2Ray/main/rules/gfwlist.conf" target="_blank"></a>'},
					{ title: '大陆白名单IP段数量', rid:'chn_number_1', text:'<a id="chn_number" href="https://raw.githubusercontent.com/HEXtoDEC/LEDE_V2Ray/main/rules/chnroute.txt" target="_blank"></a>'},
					{ title: '国内域名数量（cdn名单）', rid:'cdn_number_1', text:'<a id="cdn_number" href="https://raw.githubusercontent.com/HEXtoDEC/LEDE_V2Ray/main/rules/cdn.txt" target="_blank"></a>'},
					{ title: '规则自动更新', multi: [
